...
function requestReport_(payload) {
  ...
  if (isAdmin && !targetEmail) {
    // [REZAGADA] El indice ya NO filtra por 'SI'. El docente necesita poder
    // seleccionar un reporte SIN liberar para poder calificarlo; si se filtran,
    // la liberacion diferida es imposible (ver docs/decisiones/003).
    // Los pendientes se marcan en el nombre para distinguirlos en el desplegable.
    const reports = rows
      .map(item => {
        let section = '';
        try { section = String(JSON.parse(String(item[8] || '{}')).section || ''); } catch (_) {}
        const liberado = String(item[7] || '').trim().toUpperCase() === 'SI';
        return {
          email: String(item[1] || '').trim().toLowerCase(),
          name: (liberado ? '' : 'PENDIENTE · ') + String(item[2] || '').trim(),
          section: section,
